...
function generateCareRecommendation(
  ...
  if (precipitation > 5) {
    tips.push("Skip watering today — rainfall is sufficient for most plants.");
  } else if (temp > 35) {
    tips.push("Water plants early morning or late evening to reduce evaporation.");
  } else if (humidity < 30) {
    tips.push("Low humidity — increase watering frequency and consider misting delicate plants.");
  } else {
    tips.push("Good conditions for garden maintenance and care activities.");
  }

  if (temp > 30) {
    tips.push("Protect sensitive plants from intense heat with shade cloth.");
  }
  if (weatherCode >= 61 && weatherCode <= 67) {
    tips.push("Rainy conditions — good time to apply liquid fertilizers and check drainage.");
  }
  if (weatherCode >= 95) {
    tips.push("Storm expected — secure tall plants and move potted plants to shelter.");
  }
  if (temp < 5) {
    tips.push("Cold temperatures — protect frost-sensitive plants and reduce watering.");
  }
...
router.get("/weather", async (req, res): Promise<void> => {
  ...
  url.searchParams.set(
    "hourly",
    "temperature_2m,precipitation,weather_code,relative_humidity_2m"
  );
  url.searchParams.set(
    "daily",
  ...
  if (!response.ok) {
    req.log.error({ status: response.status }, "Open-Meteo API error");
  ...
  // Get next 24 hourly entries from current time
  const nowHour = new Date().toISOString().slice(0, 13);
  const hourlyStart = data.hourly.time.findIndex((t) => t >= nowHour);
  const hourlySlice = {
  ...
  res.json({
    ...
    hourly: hourlySlice.time.map((t, i) => ({
      time: t,
