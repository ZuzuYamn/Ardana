...
import { Cloud, CloudRain, Sun, Wind, Droplets, MapPin, Search, Calendar, Sunrise, Sunset, Leaf } from 'lucide-react';
...
export default function Weather() {
  ...
  const [coords, setCoords] = useState({ lat: 33.89, lon: 35.50, name: 'Beirut' });
    ...
    <div className="space-y-8 pb-12 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Forecasts and care advice tailored for your farm.</p>
        </div>
        ...
        <div className="space-y-8">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-8">
              <div>
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-lg font-medium tracking-wide">{weather.locationName}</span>
                </div>
                <div className="flex items-baseline gap-4">
                  ...
                  <div className="pb-2">
                    <p className="text-2xl font-medium capitalize">{weather.current.weatherDescription}</p>
                    <p className="text-white/70">Feels like {weather.current.feelsLike}°</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-4 bg-black/20 p-6 rounded-2xl backdrop-blur-md">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs text-white/60 uppercase tracking-wider">Wind</p>
                    <p className="font-semibold text-lg">{weather.current.windSpeed} km/h</p>
                  </div>
                </div>
                {weather.daily[0] && (
                  <>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-lg">{weather.daily[0].sunrise}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-lg">{weather.daily[0].sunset}</p>
                      </div>
                    </div>
                  </>
                )}
          ...
          <Card className="border-primary/20 bg-primary/5 shadow-sm">
            <CardContent className="p-6 flex gap-4 items-start">
              ...
              <div>
                <h3 className="font-serif font-bold text-lg mb-1 text-foreground">AI Care Advice</h3>
                <p className="text-foreground/80 leading-relaxed">{weather.recommendation}</p>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              ...
              <div className="h-[250px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={weather.hourly.slice(0, 24)} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <defs>
          ...
          {/* 7-Day Forecast */}
          <div>
             <h3 className="font-serif font-bold text-2xl mb-6">7-Day Forecast</h3>
             <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
               {weather.daily.map((day, i) => (
                 <Card key={i} className="text-center shadow-sm border-border hover-elevate transition-all">
                   <CardContent className="p-4 flex flex-col items-center">
                     <p className="font-medium text-foreground mb-3">{i === 0 ? 'Today' : day.date}</p>
                     {getWeatherIcon(day.weatherCode, "w-8 h-8 text-primary mb-3")}
...
// Ensure CloudSun is imported at the top
